...
*/
exports.handleListChanges = function(changedTiddlers,force) {
	// Get the new list
		...
		if(this.counterName) {
			...
			for(t=0; t<this.list.length; t++) {
				if(hasRefreshed || !this.children[t] || this.children[t].parseTreeNode.itemTitle !== this.list[t]) {
					...
					this.hasBeenRefreshed = true;
					var refreshed = this.children[t].refresh(changedTiddlers,force);
...
exports.prototype.refresh = function(changedTiddlers,force) {
	...
	if(this.storyview && this.storyview.refreshStart) {
		this.storyview.refreshStart(changedTiddlers,changedAttributes,force);
	}
	...
	if(changedAttributes.filter || changedAttributes.variable || changedAttributes.counter || changedAttributes.template || changedAttributes.editTemplate || changedAttributes.join || changedAttributes.emptyMessage || changedAttributes.storyview || changedAttributes.history) {
		...
		result = this.handleListChanges(changedTiddlers,force);
	...
	if(this.storyview && this.storyview.refreshEnd) {
		this.storyview.refreshEnd(changedTiddlers,changedAttributes,force);
	}
	if(force && !this.hasBeenRefreshed) {
		return this.refreshChildren(changedTiddlers,force) || result;
	}
	return result;
};
